...
//            $statement = $connection->query("INSERT INTO `users` (`name`, `email`, `password`) VALUES ('$name', '$email','$heshPassword') ");
            $statement = $connection->prepare("INSERT INTO `users` (`name`, `email`, `password`) VALUES (':name', ':email',':heshPassword') ");
            $statement->bindParam([':name' => $name, ':email' => $email, ':hashPassword' => $heshPassword]);
            $statement->execute();
            $statement->connection = null;
